...
public abstract class AbstractStringFieldDataTestCase extends AbstractFieldDataImplTestCase {
    ...
    public void testGlobalOrdinalsGetRemovedOnceIndexReaderCloses() throws Exception {
        ...
        assertThat(indicesFieldDataCache.getCache().policy().eviction().get().getMaximum(), equalTo(hasDocValues() ? 1L : 4L));
        ...
        assertThat(indicesFieldDataCache.getCache().policy().eviction().get().getMaximum(), equalTo(hasDocValues() ? 0L : 3L));

        refreshReader();
        assertThat(ifd.loadGlobal(topLevelReader), not(sameInstance(globalOrdinals)));

        ifdService.clear();
        assertThat(indicesFieldDataCache.getCache().policy().eviction().get().getMaximum(), equalTo(0L));
    }
}
